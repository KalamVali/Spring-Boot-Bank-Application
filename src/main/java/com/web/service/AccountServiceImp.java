...
import com.web.model.Account.AccountStatus;
...
public class AccountServiceImp implements AccountService {
	...
	public Account getBalance(int account_no,String name,String password) {
		...
		if(account_no == getAccount.getAccount_no() && name.equals(getAccount.getName()) && password.equals(getAccount.getPassword()) && getAccount.getStatus() == AccountStatus.active)
		{
			return getAccount;
		}
		else
	...
	public AmountResult withdrawAmount(int account_no,String name,String password,double amount) {
		Account getAccount = repo.findById(account_no).get();
		if(account_no == getAccount.getAccount_no() && name.equals(getAccount.getName()) && password.equals(getAccount.getPassword()) && amount < getAccount.getAmount() && getAccount.getStatus() == AccountStatus.active)
		{
			double prevBalance = getAccount.getAmount();
			double currentBalance = prevBalance - amount;
			getAccount.setAmount(currentBalance);
	...
	public TransferDetails transferAmount(int account_no,String name,String password,int targetAccount,double amount) {
		...
		if(account_no == caccount.getAccount_no() && name.equals(caccount.getName()) && password.equals(caccount.getPassword()) && targetAccount == taccount.getAccount_no() && amount < caccount.getAmount()&& caccount.getStatus() == AccountStatus.active && taccount.getStatus() == AccountStatus.active)
		{
